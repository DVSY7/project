    ...
    return (
        <React.Fragment key={"안녕"}>
            ...
            {chattingList.map((info,idx) => {
                ...
                console.log(info);
                return (
                    <div key={info.title} onClick={() => { handleChangeList(info.id) }} className={`w-[90%] h-[110px] ${isSelectedList ? "bg-gray-200" : "bg-white"} ${isSelectedTab ? "block" : "hidden"} rounded-lg border border-solid border-gray-300 mb-2`}>
                        ...
                        <div className={`flex  w-[full] h-[calc(100%_-_40px)]`}>
                            ...
                            <div className={`${flexCenter} h-full w-[80px]`}>
                                <img src={`${info.profile_image_url}`} alt='미니프로필' className={` w-[60px] h-[60px] rounded-[50%] bg-cover bg-center`}></img>
                            </div>
                            ...
                            <div className={`flex flex-col justify-between items-end w-[80px] h-full font-sans text-[0.8rem] pb-2 pr-2`}>
                                ...
                                <div>{info.last_message_time}</div>
                            </div>
                        </div>
                    </div>
                )
