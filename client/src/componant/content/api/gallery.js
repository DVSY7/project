...
export async function galleryfetch(page = 1, limit = 15, sort, searchUser) {
    ...
    try {
        const response = await axios.get(
            `${process.env.REACT_APP_SERVER_URL}/api/gallery?page=${page}&limit=${limit}&sort=${isSort}&searchUser=${searchUser}`
        );
...
export const fetchComment = async (galleryID, userID) =>{
    try{
        if(galleryID){
            const res = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/gallery/comments?galleryID=${galleryID}&userID=${userID}`);
...
export const updateComment = async(galleryID, userID, commentText) =>{
    try{
        const res = await axios.post(`${process.env.REACT_APP_SERVER_URL}/api/users/gallery/updateComment`,{
            galleryID,
            userID,
            commentText
        });
